const global = {
  ...
  api: {
    apiKey: '53f4cb1bb7477000da8fc0932f66aa79',
    ...
    <div class="details-bottom">
      ...
      <ul>
        ...
        <li><span class="text-secondary">Revenue:</span> ${
          movie.revenue ? `$${movie.revenue.toLocaleString('en-US')}` : `N/A`
        }</li>
        <li><span class="text-secondary">Runtime:</span> ${toHoursAndMinutes(
          movie.runtime
        )}</li>
...
function displaySearchResults(results) {
  ...
  results.forEach((result) => {
    ...
    document.querySelector('#search-results-heading').innerHTML = `
      <h2>
        ${results.length} of ${global.search.totalResults} Results for
        ${global.search.term}
      </h2>
    `;
...
function toHoursAndMinutes(totalMinutes) {
  ...
  return hours >= 1
    ? hours >= 2
      ...
      : `${hours} hour ${minutes > 0 ? `and ${minutes} minutes` : ''}`
    : `${minutes} minutes`;
}
